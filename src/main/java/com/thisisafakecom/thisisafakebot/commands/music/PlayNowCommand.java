...
public class PlayNowCommand extends CommandAbstract {
  ...
  private void loadAndPlay(final TextChannel channel, final String trackUrl) {
    ...
    mh.getAudioPlayerManager().loadItemOrdered(musicManager, trackUrl, new AudioLoadResultHandler() {
      ...
      public void playlistLoaded(AudioPlaylist playlist) {
        ...
        channel.sendMessage(
            "Playing now: " + firstTrack.getInfo().title + " (first track of playlist " + playlist.getName() + ")\n"
                + "Use -play [playlist url] to add all the songs in the playlist into the queue!")
            .queue();
  ...
  public String commandDescription() {
    String ret = "Immediately Plays a song from a youtube URL or searches for one off given keywords.\n" + "Usage: "
        + App.botPrefix + commandHandled + " [youtube URL OR search terms]";
    return ret;
  }
}
